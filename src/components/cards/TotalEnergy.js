      ...
      totalEnergyBranchData.devices.forEach((data) => {
        ...
        if (data.is_source && data.source_tag == "AGGREGATORY") {
          ...
          totalData.solar_hours.value =
            dashboard.solar_hours.value + (totalData.solar_hours.value || 0);
    ...
    <article className="dashboard__total-energy dashboard__banner--small">
      ...
      >
        <Tooltip
          ...
          title={DASHBOARD_TOOLTIP_MESSAGES.TOTAL_ENERGY}
        >
          <p>
      ...
      <p className="total-energy_value">
        <span>
          {totalEnergyData.total_kwh &&
            numberFormatter(totalEnergyData.total_kwh.value)}
        </span>
        <span>
          {totalEnergyData.total_kwh && totalEnergyData.total_kwh.unit}
        </span>
      </p>
      {userData.client_type !== "RESELLER" && (
        <p className="total-energy_value solar-energy_value">
          <span>
            Solar Hours:{" "}
            {totalEnergyData.solar_hours &&
              numberFormatter(totalEnergyData.solar_hours?.value)}{" "}
          </span>
          <span>
            {totalEnergyData.solar_hours && "kWh"}
            {"("}
            {(
              (totalEnergyData.solar_hours?.value /
                totalEnergyData.total_kwh?.value) *
              100
            )?.toFixed(2)}
            {"%)"}
          </span>
