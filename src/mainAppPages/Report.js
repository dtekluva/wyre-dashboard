...
function Report() {
  ...
  const bandCategorizationData = reportData.utility_band_categorization.map(
    (band, index) => ({
      ...
      deviation: `${
        parseInt(band.expected_hours) - parseInt(band.total_hours)
      }`,
      compliance: `${(
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: (context) => {
            const value = context.raw;
            return `${value}% ${
              context.datasetIndex === 0 ? "Completed" : "Remaining"
            }`;
          },
        },
      },
                  ...
                  dataSource={[
                    {
                      ...
                      value: `${reportData.solar_hour.value.toLocaleString()}${
                        reportData.solar_hour.unit
                      }`,
                    },
                    {
                      key: "1",
